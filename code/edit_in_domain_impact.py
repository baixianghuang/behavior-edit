...
if __name__ == "__main__":
    ...
    cache_dir = os.path.join(args.results_dir, 'cache_pre_edit', f'{model_name_abbrev}_moralchoice.csv')
    if os.path.exists(cache_dir):
        ...
        acc_pre, _, _, abstention_rate_pre = eval_acc_abstention(None, tokenizer, eval_prompts, eval_targets, labels, responses=responses_pre)
    else:
        model_pre = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype='auto').to(args.device_pre)
        acc_pre, responses_pre, responses_norm_pre, abstention_rate_pre = eval_acc_abstention(model_pre, tokenizer, eval_prompts, eval_targets, labels)
    ...
    for i in edit_indices:
        ...
        # acc_post, responses_post, responses_norm_post = eval_accuracy(model_post, tokenizer, eval_prompts, eval_targets, labels, responses=None, edited_idx=i)
        acc_post, responses_post, responses_norm_post, abstention_rate_post = eval_acc_abstention(model_post, tokenizer, eval_prompts, eval_targets, labels, responses=None)
        ...
        responses_path = os.path.join(args.results_dir, args.eval_data_name, f'responses_{editing_method}_{model_name_abbrev}_{i}.csv')
        ...
        with open(os.path.join(args.results_dir, 'accuracy_results.txt'), 'a') as f:
            f.write(f'Edit {i}, eval_size={eval_size} : Moral Accuracy pre: {acc_pre} -> post: {acc_post} Percent responses changed: {pct_changed:.2f}% . Abstention rate pre: {abstention_rate_pre} -> post: {abstention_rate_post}.\n')
