...
if __name__ == "__main__":
    ...
    if os.path.exists(cache_dir):
        ...
        acc_pre, _, _, abstention_rate_pre = eval_acc_abstention(None, tokenizer, model_eval, tok_eval, eval_prompts, eval_targets, labels, responses=responses_pre)
    else:
        model_pre = AutoModelForCausalLM.from_pretrained(model_id, torch_dtype='auto').to(args.device_pre)
        acc_pre, responses_pre, responses_norm_pre, abstention_rate_pre = eval_acc_abstention(model_pre, tokenizer, model_eval, tok_eval, eval_prompts, eval_targets, labels)
    ...
    for i in edit_indices:
        ...
        acc_post, responses_post, responses_norm_post, abstention_rate_post = eval_acc_abstention(model_post, tokenizer, model_eval, tok_eval, eval_prompts, eval_targets, labels)
    ...
    responses_path = os.path.join(args.results_dir, args.eval_data_name, f'responses_{editing_method}_{model_name_abbrev}_{args.steer_direction}_{eval_size}.csv')
